...
static BRESULT fileappend(CCFILEH fh, UINT32 ptr, UINT size) {

	UINT	filesize;
	UINT	r;
	UINT8	work[0x400];

	filesize = ccfile_getsize(fh);
	if (filesize < ptr) {
		return(FAILURE);
	}
	filesize -= ptr;
	while(filesize) {
		...
		filesize -= r;
		ccfile_seek(fh, ptr + filesize, FSEEK_SET);
		r = ccfile_read(fh, work, r);
		ccfile_seek(fh, ptr + filesize + size, FSEEK_SET);
		ccfile_write(fh, work, r);
	}
	return(SUCCESS);
}
...
static BRESULT writetrack(FDDFILE fdd, REG8 media, UINT track,
	...
	ccfile_seek(fh, 0, FSEEK_SET);
	ccfile_write(fh, &fdd->inf.d88.head, sizeof(fdd->inf.d88.head));
	ccfile_write(fh, ptr, sizeof(ptr));
	ccfile_seek(fh, curptr, FSEEK_SET);
	ccfile_write(fh, buf, size);
	ccfile_close(fh);
	return(SUCCESS);
}
...
BRESULT fddd88_set(FDDFILE fdd, const OEMCHAR *fname) {
	...
	r = (ccfile_read(fh, &fdd->inf.d88.head, sizeof(fdd->inf.d88.head))
											!= sizeof(fdd->inf.d88.head)) ||
		(ccfile_read(fh, ptr, sizeof(ptr)) != sizeof(ptr));
	ccfile_close(fh);
	if (r) {
		goto fdst_err;
	}
