...
PFILEH profile_open(const OEMCHAR *filename, UINT flag) {
	...
	if (fh != _FILEH_INVALID) {
		if (_file_read(fh, ret + 1, filesize) != filesize) {
			goto pfore_err3;
		}
		_file_close(fh);
	}
...
void profile_close(PFILEH hdl) {
	...
	if (hdl) {
		if (hdl->flag & PFILEH_MODIFY) {
			fh = _file_create(hdl->path);
			if (fh != _FILEH_INVALID) {
				if (hdl->flag & PFILEH_UTF8) {
					_file_write(fh, utf8header, 3);
				}
				_file_write(fh, hdl->buffer, hdl->size);
				_file_close(fh);
			}
		}
		_MFREE(hdl);
